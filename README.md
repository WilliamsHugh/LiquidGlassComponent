...
The current material adds a superellipse lens profile, Fresnel-weighted refraction and a geometry-derived rim highlight. The previous page is preserved at [milestones/v1/index.html](milestones/v1/index.html). Open [compare.html](compare.html) for V1/V2 side-by-side. See [OPTICS-V2.md](OPTICS-V2.md) for the optical model, validation, sources and limitations. `npm test` checks optical invariants; browser comparison screenshots are in `artifacts/`.
