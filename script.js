...
const passcodeDots = document.querySelectorAll('#passcode-dots i');
const passcodeStatus = document.querySelector('#passcode-status');
function renderPasscode() {
  passcodeDots.forEach((dot, index) => dot.classList.toggle('filled', index < passcodeDigits.length));
  document.querySelector('#passcode-dots').setAttribute('aria-label', `${passcodeDigits.length} of 4 digits entered`);
}
document.querySelectorAll('.key[data-key]').forEach(button => button.addEventListener('click', () => {
  if (passcodeDigits.length >= 4) return;
  passcodeDigits.push(button.dataset.key);
  passcodeStatus.textContent = passcodeDigits.length === 4 ? 'Ready to unlock your space.' : `${4 - passcodeDigits.length} more ${4 - passcodeDigits.length === 1 ? 'tap' : 'taps'} to go.`;
  passcodeStatus.className = 'passcode-status';
  renderPasscode();
}));
document.querySelector('#clear-passcode').addEventListener('click', () => {
  passcodeDigits.length = 0;
  passcodeStatus.textContent = 'Your space is just four taps away.';
  passcodeStatus.className = 'passcode-status';
  renderPasscode();
});
document.querySelector('#submit-passcode').addEventListener('click', () => {
  if (passcodeDigits.length !== 4) {
    passcodeStatus.textContent = 'Enter four digits first.';
    passcodeStatus.className = 'passcode-status error';
    return;
  }
  passcodeStatus.textContent = passcodeDigits.join('') === '2580' ? 'Welcome back. Your space is open.' : 'That code is not quite right. Try again.';
  passcodeStatus.className = `passcode-status ${passcodeDigits.join('') === '2580' ? 'success' : 'error'}`;
});
...
function setTheme(dark) {
  ...
  themeToggle.querySelector('.theme-sun').hidden = dark;
  themeToggle.querySelector('.theme-moon').hidden = !dark;
  try { localStorage.setItem('aera-theme', dark ? 'dark' : 'light'); } catch {}
}
themeToggle.addEventListener('click', () => setTheme(!document.body.classList.contains('theme-dark')));
try { setTheme(localStorage.getItem('aera-theme') === 'dark'); } catch { setTheme(false); }
